...
namespace BlackCat\Database\Packages\RbacSyncCursors\Joins;
 ...
 */
final class RbacSyncCursorsJoins {
    ...
    /**
     * FK: rbac_sync_cursors -> rbac_repositories
     * LEFT JOIN vw_rbac_repositories AS $as ON $as.id = $alias.repo_id
     * @return array{0:string,1:array<string,mixed>}
     */
    public function joinRbacRepositories(string $alias = 't', string $as = 'j0'): array {
        [$alias, $as] = $this->assertAliasPair($alias, $as);
        return [' LEFT JOIN vw_rbac_repositories AS ' . $as . ' ON ' . $as . '.id = ' . $alias . '.repo_id' . ' ', []];
    }
