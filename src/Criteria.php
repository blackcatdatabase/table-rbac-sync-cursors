...
final class Criteria extends BaseCriteria
{
    ...
    {
        $x = [ 'repo_id', 'peer', 'last_commit', 'last_synced_at' ];
    ...
    public static function fromDb(
        ...
        if (\method_exists(\BlackCat\Database\Packages\RbacSyncCursors\Definitions::class, 'softDeleteColumn')) {
            $soft = \BlackCat\Database\Packages\RbacSyncCursors\Definitions::softDeleteColumn();
    ...
    public function byId(int|string $id): self {
        return $this->where('t.repo_id peer = :cid', ['cid' => $id]);
    }
    public function byIds(array $ids): self {
        if (!$ids) return $this->where('1=0');
        return $this->whereIn('t.repo_id peer', array_values($ids));
    }
