...
namespace BlackCat\Database\Packages\RbacSyncCursors\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\RbacSyncCursors\Dto\RbacSyncCursorDto;
use BlackCat\Database\Packages\RbacSyncCursors\Mapper\RbacSyncCursorDtoMapper;
 ...
 */
final class RbacSyncCursorsAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private RbacSyncCursorRepository $rbacSyncCursorRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'rbac_sync_cursors' => $this->rbacSyncCursorRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\RbacSyncCursors\ModuleException("Record {$idStr} not found for locking.");
            }
