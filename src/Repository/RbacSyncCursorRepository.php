...
namespace BlackCat\Database\Packages\RbacSyncCursors\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\RbacSyncCursors\Definitions;
use BlackCat\Database\Packages\RbacSyncCursors\Criteria;
use BlackCat\Database\Packages\RbacSyncCursors\Dto\RbacSyncCursorDto as Dto;
use BlackCat\Database\Packages\RbacSyncCursors\Mapper\RbacSyncCursorDtoMapper as RowMapper;
...
final class RbacSyncCursorRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\RbacSyncCursors\Definitions::class; }
    ...
    private function doUpsert(array $row, bool $revive): void
    {
        ...
        $updCols = [ 'last_commit', 'last_synced_at' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'repo_id', 'peer' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\RbacSyncCursors\Definitions::table());
            return $bulk->upsertMany($rows, [ 'repo_id', 'peer' ], [ 'last_commit', 'last_synced_at' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\RbacSyncCursors\Dto\RbacSyncCursorDto|null */
    public function getByRepoIdAndPeer(int $repoId, string $peer, bool $asDto = false): array|\BlackCat\Database\Packages\RbacSyncCursors\Dto\RbacSyncCursorDto|null {
        $row = $this->getByUnique([ 'repo_id' => $repoId, 'peer' => $peer ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\RbacSyncCursors\Mapper\RbacSyncCursorDtoMapper::fromRow($row);
    }
    public function existsByRepoIdAndPeer(int $repoId, string $peer): bool {
        $where = 't.' . Ident::q($this->db, 'repo_id') . ' = :uniq_repo_id' . ' AND ' . 't.' . Ident::q($this->db, 'peer') . ' = :uniq_peer';
        return $this->exists($where, [ 'uniq_repo_id' => $repoId, 'uniq_peer' => $peer ]);
    }
    /** @return int|string|null */
    public function getIdByRepoIdAndPeer(int $repoId, string $peer) {
        $row = $this->getByRepoIdAndPeer($repoId, $peer);
        return $row ? ($row['repo_id peer'] ?? null) : null;
    }
