...
use BlackCat\Database\Packages\RbacSyncCursors\Repository\RbacSyncCursorRepositoryInterface;
...
class RbacSyncCursorRepository implements RbacSyncCursorRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'repo_id', 'peer' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\RbacSyncCursors\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'last_commit', 'last_synced_at' ]);
          return count($rows);
        }
    ...
    public function getByRepoIdAndPeer(int $repoId, string $peer, bool $asDto = false): array|\BlackCat\Database\Packages\RbacSyncCursors\Dto\RbacSyncCursorDto|null {
        return $this->getByUnique([ 'repo_id' => $repoId, 'peer' => $peer ], $asDto);
    }
    ...
    public function getIdByRepoIdAndPeer(int $repoId, string $peer) {
        $row = $this->getByRepoIdAndPeer($repoId, $peer, false);
        if (!is_array($row)) { return null; }
        return $row['repo_id peer'] ?? null;
    }
